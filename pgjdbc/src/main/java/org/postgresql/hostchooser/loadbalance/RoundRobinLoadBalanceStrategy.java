 ...
 * <b>Characteristics:</b>
 * <ul>
 *   <li>Stateful - maintains a counter per cluster using {@link ClusterManager}</li>
...
public class RoundRobinLoadBalanceStrategy implements LoadBalanceStrategy {
  ...
  /**
   * Constructs a round-robin strategy using the specified cluster manager.
   *
   * @param clusterManager Manager for maintaining cluster counters
   */
  public RoundRobinLoadBalanceStrategy(ClusterManager clusterManager) {
  ...
  public List<HostSpec> orderHosts(List<HostSpec> hosts, String clusterId) {
    ...
    int counter = clusterManager.getAndIncrementCounter(clusterId);
